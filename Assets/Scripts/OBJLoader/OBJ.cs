...
public class OBJ
{
	...
	private void Init()
	{
		...
		coroutiner.AddComponent<Coroutiner>();
	}
    ...
    public void Start(string objPath, Vector3 translation, Action callback)
    {
		...
		WWW loader = new WWW(normalPath);
		coroutiner.GetComponent<Coroutiner>().StartCoroutine(Load(loader, normalPath, translation, callback));
    }
    ...
    public IEnumerator Load(WWW loader,string path, Vector3 translation, Action callback)
    {
        ...
        if (hasMaterials)
        {
            WWW MTLloader = new WWW(basepath + mtllib);
            coroutiner.GetComponent<Coroutiner>().StartCoroutine(LoadMTL(MTLloader, translation, callback));
        }
		else
		{
        	Build(translation);
	...
	public IEnumerator LoadTexture(WWW loader, MaterialData m, string text, Vector3 translation, Action callback)
    {
        ...
        {
			if (text.Equals("diffuse"))
			{
				m.diffuseTex = loader.texture;
			}
			else
			{
				m.bumpTex = loader.texture;
			}
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            switch (p[0])
            {
                ...
                case UML:
                    if (p.Length < 2)
                    {
                        buffer.PushMaterialName("Material".Trim());
                    }
                    else
                    {
        ...
        for (int i = 0; i < lines.Length; i++)
        {
            ...
            switch (p[0])
            {
                ...
                case KS:
                    ...
                    break;
                case NS:
                    current.shininess = /*cf(p[1])*/1000f / 1000;
                    break;
                case D:
                case TR:
    ...
    private Material GetMaterial(MaterialData md)
    {
        ...
        if (md.illumType == 2)
        {
            string shaderName = (md.bumpTex != null) ? "Bumped Specular" : "Self-Illumin/Specular";
            m = new Material(Shader.Find(shaderName));
            m.mainTextureScale = new Vector2(2.0f, 2.0f);
    ...
    private void Build(Vector3 translation)
    {
        ...
        if (buffer.numObjects == 1)
        {
            ...
            myGo.AddComponent(typeof(MeshFilter));
            myGo.AddComponent(typeof(MeshRenderer));
			myGo.transform.Translate(translation);
            ms[0] = myGo;
        }
        ...
        buffer.PopulateMeshes(ms, materials, _first);
		_first = false;
    }
}
